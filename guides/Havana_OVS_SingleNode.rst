...
:Version: 2.0
:Source: https://github.com/mseknibilel/OpenStack-Havana-Install-Guide
:Keywords: Single node OpenStack, Havana, Neutron, Nova, Keystone, Glance, Horizon, Cinder, OpenVSwitch, KVM, Ubuntu Server 12.04 (64 bits).
...
* Fill up the keystone database using the two scripts available in the `Scripts folder <https://github.com/mseknibilel/OpenStack-Havana-Install-Guide/tree/master/KeystoneScripts>`_ of this git repository::
   ...
   wget https://raw.github.com/mseknibilel/OpenStack-Havana-Install-Guide/OVS_SingleNode/KeystoneScripts/keystone_basic.sh
   wget https://raw.github.com/mseknibilel/OpenStack-Havana-Install-Guide/OVS_SingleNode/KeystoneScripts/keystone_endpoints_basic.sh
...
To create the neutron external network you should then follow `the multinode guide's section 5 <https://github.com/mseknibilel/OpenStack-Havana-Install-Guide/blob/OVS_MultiNode/OpenStack_Havana_Install_Guide.rst#5-your-first-vm>`_ on this. Note: when creating the external network, be sure to set the gateway IP to 192.168.100.51
...
* Install the neutron components::
   ...
   apt-get install -y neutron-server neutron-plugin-openvswitch neutron-plugin-openvswitch-agent dnsmasq neutron-dhcp-agent neutron-l3-agent 
...
* Check the status of all nova-services::
   ...
   for i in $( ls /etc/init.d/nova-* | xargs -n 1 basename ); do service $i status; cd; done
